...
export function MathFingerLogo({ 
  ...
  return (
    <div className="flex items-center gap-3">
      {/* Custom Image Logo with SVG Fallback */}
      {!imgError ? (
        <img 
          src="/logo.png" 
          width={size} 
          ...
          className="rounded-xl object-cover border border-emerald-400/50 bg-white transform transition-transform hover:scale-110 duration-200 cursor-pointer shadow-sm"
          style={{ width: size, height: size, minWidth: size, minHeight: size }}
        />
      ) : (
        /* Playful smiling hand SVG Fallback */
        <svg 
          width={size} 
          height={size} 
          viewBox="0 0 260 260" 
